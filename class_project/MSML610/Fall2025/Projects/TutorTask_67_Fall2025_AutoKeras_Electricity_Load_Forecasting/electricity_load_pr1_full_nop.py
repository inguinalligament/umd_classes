...
# -*- coding: utf-8 -*-
"""
PR1 — Electricity Load Forecasting (Full, Safe Flags)
-----------------------------------------------------
Same as `electricity_load_pr1_full.py` but with model toggles:
  --no-arima     Disable ARIMA
  --no-prophet   Disable Prophet (useful on macOS if you see "zsh: bus error")
  --no-lstm      Disable LSTM

Run example:
python electricity_load_pr1_full_safe.py --file "/path/to.csv" --timestamp-col "datetime" --target-col "nat_demand" --no-prophet

If you want to try Prophet on macOS/Apple Silicon:
  pip install "prophet==1.1.5" "cmdstanpy==1.2.3" "numpy<2"
Or use: pip install prophet  (may work on Intel/conda envs)
"""
import argparse
import sys
...
def main():
    ...
    p.add_argument("--timestamp-col", type=str, default="")
    p.add_argument("--target-col", type=str, default="")
    ...
    candidate_paths = [Path(args.file)] if args.file else []
    candidate_paths += [
    ...
    file_path = next((p for p in candidate_paths if p and p.exists()), None)
    ...
    naive_24 = df[target_col].shift(24).reindex(test_df.index).fillna(method="bfill").fillna(method="ffill")
    ...
    if not args.no_arima:
        try:
            ...
            pred_arima = np.asarray(arima_fit.forecast(steps=len(test_df)), dtype=float)
    ...
    outdir = Path(args.outdir) if args.outdir else (file_path.parent if file_path.parent.exists() else Path("./outputs"))
    ...
    best_col = {
        "seasonal_naive_24": "y_hat_naive24",
        "linear_regression": "y_hat_lr",
        "ridge": "y_hat_ridge",
        "random_forest": "y_hat_rf",
        "arima": "y_hat_arima",
        "prophet": "y_hat_prophet",
        "lstm": "y_hat_lstm",
    }[best_model]
    best_pred = pred_df[best_col].values
